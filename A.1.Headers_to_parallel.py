...
# with the "A.2.Parallel_FASTA_maker.sh" script to generate the corresponding FASTA files starting from a single big file.
# ----------------------------------------------------------------------------------------------------------------------
#
# OUTPUTS --------------------------------------------------------------------------------------------------------------
...
def header_formatter(usr_metadata, threads, gisaid_type, index_column, header_format, outfolder):
	...
	with open(f"{path_out}CL_squared_parallel/Summary.txt", "w") as summary:
		...
		cleaned_meta = pl.read_csv(usr_metadata, separator="\t", quote_char=None, ignore_errors=True).fill_null(value="Undefined")
		...
		if type == "Gisaid":
			...
			if gisaid_type == "A" and "Headers" not in meta_columns:
			...
			elif gisaid_type == "B" and "Headers" not in meta_columns:
			...
			cleaned_meta = cleaned_meta.with_columns(pl.col("Accession").str.replace_all(" ", "_"))

			all_seqs = list(cleaned_meta.get_column("Accession"))

			if "Headers" not in cleaned_meta.columns:

				cleaned_meta = cleaned_meta.with_columns(pl.Series(name="Headers", values=all_seqs))
			...
			all_seqs = [i.replace(" ", "_") for i in all_seqs]
			cleaned_meta = cleaned_meta.with_columns(pl.Series(name="Headers", values=all_seqs))
		...
		for i in range(0, len(all_seqs), number_of_sec_per_file):
			...
			if not os.path.isdir(f"{path_out}CL_squared_parallel/Headers_Groups_{threads}"):
				os.makedirs(f"{path_out}CL_squared_parallel/Headers_Groups_{threads}")

			with open(f"{path_out}CL_squared_parallel/Headers_Groups_{threads}/Headers_group_{j}.txt", "w") as doc:
				for el in group:
					doc.write(f"{el}\n")
...
parser = argparse.ArgumentParser(description="This script exploits sequences metadata to generate .txt files containing"
                                             ...
                                             "of this script will correspond to the number of threads that the user wants to "
                                             "use. The script MUST BE JOINTLY USED with the \"A.2.Parallel_FASTA_maker.sh\" script "
                                             "to generate the FASTA files starting from a single big file.",
                                             formatter_class=RawTextHelpFormatter)

# User metadata file
parser.add_argument("usr_metadata", type=str, help="Metadata file path.\nIf you work with NCBI metadata, please download "
                                                   "the accession ID WITH VERSION\nIf you have multiple metadata files, "
                                                   "concatenate or merge them.\nMANDATORY FORMAT: .tsv\nMandatory columns:\n"
                                                   "Accession ID, Collection Date, Submission date, Virus name (GISAID)"
...
parser.add_argument("--header_format", type=str, help="If your metadata have not GISAID or Ncbi format, you can use "
                                                      ...
                                                      "like this:\n\n\"STRAIN COLUMN NAME,|,COLLECTION DATE COLUMN NAME\""
                                                      "\n\nPlease pay attention at the white spaces. Do not add white"
                                                      " spaces before or after the comma. Only use the quotes to limit the "
                                                      " whole string (\"...\"). If your headers "
