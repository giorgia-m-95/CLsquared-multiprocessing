...
#               Accession (sequence ID), Collection_Date, Country, Host (NCBI metadata)
#
    ...
    with open(f"{path_out}CL_squared_step_01/Metadata_filtering_summary.txt", "w") as doc:
        ...
        if loc != "":
            ...
            if place:
                ...
                places_list = list(set(meta.get_column(pl.col(loc).str.to_titlecase())))
        ...
        if cd != "":
            ...
            if completeness == "n":
                ...
                meta = meta.with_columns(pl.col(cd).map_elements(lambda x: x + "-01-01" if len(x) == 4 else x))
                meta = meta.with_columns(pl.col(cd).map_elements(lambda x: x + "-15" if len(x) == 7 else x))
...
parser.add_argument("usr_metadata", type=str, help="Metadata file path.\nSequence ID, Collection date, Location "
                                                   ...
                                                   ", concatenate or merge them. MANDATORY FILE FORMAT: .tsv")
