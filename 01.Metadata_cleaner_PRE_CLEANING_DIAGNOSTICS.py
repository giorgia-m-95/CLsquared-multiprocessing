    ...
    with open(f"{path_out}CL_squared_step_01_DIAGNOSTICS/Metadata_pre_filtering_diagnostics.txt", "w") as doc:
        ...
        if type == "Gisaid":
            ...
            try:
                ...
                meta_temp = meta.select(pl.col(["Isolate Collection date", "Submitter Country", "Host Name"]))

                meta_temp = meta_temp.filter((pl.col('Isolate Collection date') == 'Undefined') |
                                             (pl.col('Submitter Country') == 'Undefined') |
                                             (pl.col('Host Name') == 'Undefined'))
        ...
        if cd != "":
            ...
            if start_date and end_date:
                ...
                meta_incompl = meta_corr.with_columns(pl.col(cd).map_elements(lambda x: x + "-01-01" if len(x) == 4 else x, return_dtype=str).alias("Filled_dates"))
                meta_incompl = meta_incompl.with_columns(pl.col("Filled_dates").map_elements(lambda x: x + "-15" if len(x) == 7 else x, return_dtype=str))

                meta_incompl = meta_incompl.with_columns(pl.col("Filled_dates").str.to_date("%Y-%m-%d"))
                meta_incompl = meta_incompl.filter((pl.col("Filled_dates") >= start) &
                ...
                meta_incompl = meta_incompl.with_columns(pl.col("Filled_dates").str.to_date("%Y-%m-%d"))
                meta_incompl = meta_incompl.filter(pl.col("Filled_dates") <= end)
...
parser = argparse.ArgumentParser(description="Metadata quality diagnostics.\n"
...
parser.add_argument("usr_metadata", type=str, help="Metadata file path.\nSequence ID, Collection date, Location "
                                                   "and Host columns are mandatory. If you work with NCBI metadata"
                                                   " please download the accession ID WITH VERSION\nIf you have multiple"
                                                   " metadata files, concatenate or merge them.\nMANDATORY FORMAT: tsv FILE.")
...
parser.add_argument("--meta_columns_list", type=str, help="If your metadata file doesn't match the GISAID and NCBI "
                                                          ...
                                                          "\n ATTENTION: if you have missing columns, do maintain the "
                                                          "order and use \"\" in its place. For instance, if you lack the"
                                                          " \"host\" column, your list should look like this: [\"sequence"
                                                          " id column name\", \"\", \"place column name\", \"collection "
